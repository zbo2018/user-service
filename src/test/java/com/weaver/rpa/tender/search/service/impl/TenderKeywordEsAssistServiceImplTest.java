...
import com.weaver.rpa.tender.search.service.TenderKeywordEsAssistService;
...
@SpringBootTest
public class TenderKeywordEsAssistServiceImplTest {
    @Autowired
    TenderKeywordEsAssistService tenderEsAssistService;
    ...
    public void viewAnalyzerTermsTest(){
        ...
        String content = "<p>I&apos;m so <b>happy</b>!</p>";
    ...
    public void searchTest(){
        String path = "C:\\Users\\Administrator\\Desktop\\匹配的词.txt";
        List<String> result = new ArrayList<>();
        List<String> words = ReadFromFile.readAllFile(path);
        int i=1;
        for(String word : words){
            if(i%100==0){
                System.out.println("已处理"+i+"个");
            }
            i++;
            ResponseModel<List<TenderKeywordEsDto>> resp = tenderEsAssistService.search(word);
            List<TenderKeywordEsDto> data = resp.getData();
            if(data==null || data.isEmpty()){
                continue;
            }
            List<String> filter = data.stream().map(TenderKeywordEsDto::getTag)
                    .filter(tag -> !tag.toLowerCase().equals(word.toLowerCase()))
    ...
    public void searchTest_2(){
        ...
        ResponseModel<List<TenderKeywordEsDto>> resp =tenderEsAssistService.search(content);
